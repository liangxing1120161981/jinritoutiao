...
let newslistsModel = dbconn.model("newslists",newslistsSchema);

module.exports = {
    //完成添加数据的功能
    "add": function (data, callback) {
        //创建实体（要插入到数据库中数据）
        let newslistsEntity = new newslistsModel(data);

        newslistsEntity.save((err, data)=> {
            if (err) {
                console.log(err);
                callback(false);
            } else {
    ...
    "find": function (condition, callback) {
        newslistsModel.find(condition, (err, data)=> {
            if (err) {
                console.log(err);
                callback([]);
            } else {
